...
my($me, $n,$id,$c,$id2ossl,$id2rfc,$n_ossl,$n_rfc,$ossl2id,$rfc2id,$opts);
my(@rfc_ciphers, @out, @ids);
...
while(1){
  ...
  ($id,$n)=splice (@rfc_ciphers,0,2);
  $id=uc($id);$n=uc($n);
  $rfc2id->{$n}=$id;
  $id2rfc->{$id}=$n;
}

open (OSSL, '-|', 'openssl', 'ciphers', '-V', 'ALL:eNULL') or die;
@out=<OSSL>;
close OSSL;

map {
  if (m/^\s*(0x[0-9a-f][0-9a-f],0x[0-9a-f][0-9a-f])\s*-\s*(\S+)/i){
    $id=uc($1),$n=uc($2);
    $ossl2id->{$n}=$id;
    $id2ossl->{$id}=$n;
  }
...
unless($opts->{b}){
  ...
  print STDERR "$me Unknown cipher $ARGV[0]\n";
  exit 1;
  }
}
...
sub getids($){
  my($c,$id);
  $c = shift;
  if($c =~ /^0x[0-9A-F][0-9A-F],0x[0-9A-F][0-9A-F]$/i){
    $id=$c;
  } elsif(exists($rfc2id->{$c})){
    $id=$rfc2id->{$c};
  } elsif(exists($ossl2id->{$c})){
    $id=$ossl2id->{$c}
  } else {
    return ();
  }
  $n_rfc=$id2rfc->{$id} || 'UNKNOWN';
  $n_ossl=$id2ossl->{$id} || 'UNKNOWN';
  $id =~ s/\b0X/0x/g;
  return ($id, $n_ossl, $n_rfc);
}  

sub usage(){
print STDERR
  "Usage:
  $me <CIPHER_NAME_OR_ID>
  $me -b*

  * Read ciphers/ids from stdin
";
}
